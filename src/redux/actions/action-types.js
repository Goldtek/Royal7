export const LOGIN_REQUEST = "LOGIN_REQUEST";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAILURE = "LOGIN_FAILURE";
export const LOGOUT = "LOGOUT";

export const SUCCESS = "ALERT_SUCCESS";
export const ERROR = "ALERT_ERROR";
export const CLEAR = "ALERT_CLEAR";

//CREATE SCHOOL ACCOUNT
// const CREATE_ACCOUNT_REQUEST = "CREATE_ACCOUNT_REQUEST";
// const CREATE_ACCOUNT_SUCCESS = "CREATE_ACCOUNT_SUCCESS";
// const CREATE_ACCOUNT_FAILURE = "CREATE_ACCOUNT_FAILURE";

//LIST OF TEACHERS
export const FETCH_TEACHERS_SUCCESS = "FETCH_TEACHERS_SUCCESS";
export const FETCH_TEACHERS_REQUEST = "FETCH_TEACHERS_REQUEST";
export const FETCH_TEACHERS_FAILURE = "FETCH_TEACHERS_FAILURE";
//FETCH SINGLE TEACHER
export const FETCH_SINGLE_TEACHER_SUCCESS = "FETCH_SINGLE_TEACHER_SUCCESS";
export const FETCH_SINGLE_TEACHER_REQUEST = "FETCH_SINGLE_TEACHER_REQUEST";
export const FETCH_SINGLE_TEACHER_FAILURE = "FETCH_SINGLE_TEACHER_FAILURE";

//ASSIGNED TEACHER REQUEST
export const FETCH_ASSIGNED_TEACHER_REQUEST = "FETCH_ASSIGNED_TEACHER_REQUEST";
export const FETCH_ASSIGNED_TEACHER_SUCCESS = "FETCH_ASSIGNED_TEACHER_SUCCESS";
export const FETCH_ASSIGNED_TEACHER_FAILURE = "FETCH_ASSIGNED_TEACHER_FAILURE";

//LIST OF ALL STUDENTS
export const FETCH_STUDENTS_SUCCESS = "FETCH_STUDENTS_SUCCESS";
export const FETCH_STUDENTS_REQUEST = "FETCH_STUDENTS_REQUEST";
export const FETCH_STUDENTS_FAILURE = "FETCH_STUDENTS_FAILURE";
//LIST OF  SINGLE STUDENT
export const FETCH_SINGLE_STUDENT_SUCCESS = "FETCH_SINGLE_STUDENT_SUCCESS";
export const FETCH_SINGLE_STUDENT_REQUEST = "FETCH_SINGLE_STUDENT_REQUEST";
export const FETCH_SINGLE_STUDENT_FAILURE = "FETCH_SINGLE_STUDENT_FAILURE";

//STUDENT ASSESSMENT
export const FETCH_STUDENT_ASSESSMENT_REQUEST =
  "FETCH_STUDENT_ASSESSMENT_REQUEST";
export const FETCH_STUDENT_ASSESSMENT_SUCCESS =
  "FETCH_STUDENT_ASSESSMENT_SUCCESS";
export const FETCH_STUDENT_ASSESSMENT_FAILURE =
  "FETCH_STUDENT_ASSESSMENT_FAILURE";

//UPDATE STUDENT
export const UPDATE_STUDENT_REQUEST = "UPDATE_STUDENT_REQUEST";
export const UPDATE_STUDENT_SUCCESS = "UPDATE_STUDENT_SUCCESS";
export const UPDATE_STUDENT_ERROR = "UPDATE_STUDENT_ERROR";

// CLASSESS
export const FETCH_CLASS_REQUEST = "FETCH_CLASS_REQUEST";
export const FETCH_CLASS_SUCCESS = "FETCH_CLASS_SUCCESS";
export const FETCH_CLASS_FAILURE = "FETCH_CLASS_FAILURE";
//Fetch the subject each student offers
export const FETCH_SUBJECT_STUDENTS_REQUEST = "FETCH_SUBJECT_STUDENTS_REQUEST";
export const FETCH_SUBJECT_STUDENTS_SUCCESS = "FETCH_SUBJECT_STUDENTS_SUCCESS";
export const FETCH_SUBJECT_STUDENTS_FAILURE = "FETCH_SUBJECT_STUDENTS_FAILURE";
//fetch class time table routines
export const FETCH_CLASS_ROUTINE_REQUEST = "FETCH_CLASS_ROUTINE_REQUEST";
export const FETCH_CLASS_ROUTINE_SUCCESS = "FETCH_CLASS_ROUTINE_SUCCESS";
export const FETCH_CLASS_ROUTINE_FAILURE = "FETCH_CLASS_ROUTINE_FAILURE";

// SUBJECT CONST
export const FETCH_SUBJECTS_REQUEST = "FETCH_SUBJECTS_REQUEST";
export const FETCH_SUBJECTS_SUCCESS = "FETCH_SUBJECTS_SUCCESS";
export const FETCH_SUBJECTS_FAILURE = "FETCH_SUBJECTS_FAILURE";

export const FETCH_ASSIGNED_SUBJECTS_REQUEST =
  "FETCH_ASSIGNED_SUBJECTS_REQUEST";
export const FETCH_ASSIGNED_SUBJECTS_SUCCESS =
  "FETCH_ASSIGNED_SUBJECTS_SUCCESS";
export const FETCH_ASSIGNED_SUBJECTS_FAILURE =
  "FETCH_ASSIGNED_SUBJECTS_FAILURE";

// DELETE SUBJECT
export const DELETE_SUBJECT_REQUEST = "DELETE_SUBJECT_REQUEST";
export const DELETE_SUBJECT_SUCCESS = "DELETE_SUBJECT_SUCCESS";
export const DELETE_SUBJECT_FAILURE = "DELETE_SUBJECT_FAILURE";

// EXAM ACTION TYPES
// <-- FETCH EXAMS-->
export const FETCH_EXAM_SESSION_TABLE_REQUEST =
  "FETCH_EXAM_SESSION_TABLE_REQUEST";
export const FETCH_EXAM_SESSION_TABLE_SUCCESS =
  "FETCH_EXAM_SESSION_TABLE_SUCCESS";
export const FETCH_EXAM_SESSION_TABLE_FAILURE =
  "FETCH_EXAM_SESSION_TABLE_FAILURE";

export const FETCH_EXAMS_TABLE_REQUEST = "FETCH_EXAMS_TABLE_REQUEST";
export const FETCH_EXAMS_TABLE_SUCCESS = "FETCH_EXAMS_TABLE_SUCCESS";
export const FETCH_EXAMS_TABLE_FAILURE = "FETCH_EXAMS_TABLE_FAILURE";
// <-- FETCH EXAMS-->
